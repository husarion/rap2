...
const { argv } = yargs
  ...
  .option('slam_engine', {
    alias: ['slam', 'slam-engine'],
    default: 'gmapping',
    description: 'Underlying SLAM library. Note: library needs to be installed in ROS',
    type: 'string',
  })
  .help()
  .alias('help', 'h')
...
console.log('SLAM Engine: ', argv.slam_engine);
...
function startSLAM() {
  if (argv.slam_engine === 'gmapping') {
    startGmapping();
  } else if (argv.slam_engine === 'slam_toolbox') {
    startSlamToolbox();
  } else {
    console.error('ERROR : INVALID SLAM_ENGINE', argv.slam_engine);
  }
}
